...
const (
	...
	contentTypeTexts      = `text/html`
	contentTypeAplication = `application/xml`
)
...
func (c *Client) Do(r *http.Request) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		case contentTypeTexts:
			if err := json.Unmarshal(body, &e); err != nil {
				return nil, fmt.Errorf("%w: %v", ErrParseErrorMessageFailure, string(body))
			}
		case contentTypeAplication:
			if err := json.Unmarshal(body, &e); err != nil {
				return nil, fmt.Errorf("%w: %v", ErrParseErrorMessageFailure, string(body))
			}
		default:
...
// Delete executes a HTTP PUT request.
...
// DeleteXML executes a HTTP PUT request with `application/xml` content type.
func (c *Client) DeleteXML(u *url.URL, data interface{}) ([]byte, error) {
	b, err := xml.Marshal(data)
	if err != nil {
		return nil, err
	}
	return c.Delete(u, contentTypeXML, b)
}
...
func (c *Client) Reboot() (resp *ResponseStatus, err error) {
	...
	body, err := c.PutXML(u, "")
	if err != nil {
		fmt.Println("======> Reboot Unsuccessful: ", err)
		return nil, err
	}
	err = xml.Unmarshal(body, &resp)
	if err != nil {
		fmt.Println("======> Reboot Unmarshal Unsuccessful: ", err)
		return nil, err
	}
	fmt.Println("=======> Reboot device success: ", resp)
	return resp, nil
}
